...
set targetApoapsis to 56753550. //Target apoapsis in meters
set targetPeriapsis to 40764550. //Target periapsis in meters
...
if stage:liquidfuel<1 and stage:solidfuel<1 and stage:monopropellant<1 AND runmode>1 {//staging function
		wait 0.1.
		if stage:solidfuel<1 and stage:solidfuel>=0	stage.
		else if stage:liquidfuel<1 and stage:liquidfuel>0 stage.
		else if stage:monopropellant<1 and stage:monopropellant>0 stage.
		}
if ship:altitude>50000 ag6 on. //fairing deploy, or whatever on action group 6.   
 ...
 if runmode = 1 { //Ship is on the launchpad
        ...
        unlock steering.
	sas on.
        print "IN TRANSFER ORBIT".
	SET runmode to 11.
     	}
    else if runmode = 11{
	if eta:apoapsis>150 set kuniverse:timewarp to 6.
	else if eta:apoapsis <150 {
		set kuniverse:timewarp to 0.
		if eta:apoapsis < 30 stage.
	}
	if stage:number = 0{
