//Universal Booster Landing Script: 
...
SET launchPad TO SHIP:GEOPOSITION. set count to 0.					//for landing on launchpad
//SET launchPad TO VESSEL("Of Course I Still Love You"):GEOPOSITION. //SET count to 1.	//for barge landing. Change vessel name to your ASDS name. count determines trajectory, 0 for RTLS, 1 for barge landing.
...
WHEN runMode = 6 THEN {
	...
	SET updateSettings TO true.
	WHEN (stage:liquidfuel/fullFuel)<0.15 AND STAGE:LIQUIDFUEL > 0 or ship:apoapsis>90000 THEN {//Stages when booster has 15% fuel (*just* enough to land. Falcon 9 has a value of 16% AFAIK) or when apoapsis is 90k. second case is very rare, in that case first orbit the rocket and then land.
...
UNTIL stopLoop = true { //Main loop	
	if runMode = 6 {	
		...
		if count = 0 set steeringPitch to max(50, 90 * (1 - radar / 50000)).		//more vertical ascent trajectory for RTLS.
		else if count = 1 {
			//if ship:airspeed > 300 and ship:airspeed <500 set steeringPitch to ship:prograde.
			set steeringPitch to max(25, 90 * (1 - radar / 30000)). 	//flatter ascent for droneship landing.
	}}
	if runMode = 5 { //boostback
		...
		if ADDONS:TR:HASIMPACT = true { 		//If ship will hit ground
			...
			if VANG(HEADING(steeringDir,steeringPitch):VECTOR, SHIP:FACING:VECTOR) < 20 {  //wait until pointing in right direction, saves fuel.
				SET thrott TO targetDist / 20000 + 0.1.
			} else {
				SET thrott TO 0.
			}
